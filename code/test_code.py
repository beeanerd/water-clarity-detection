...
def hough_line_drawing(images):
    ...
    for count, imgPath in enumerate(images):
        ...
        fm = cv.dilate(fm, cv.getStructuringElement(cv.MORPH_RECT, (5, 5)), iterations=3)
        fm = cv.erode(fm, cv.getStructuringElement(cv.MORPH_RECT, (5, 5)), iterations=3)
        ...
        image = resize_with_aspect_ratio(image, width=720)
